...
|----------|----------|-------|------------|
| Broadcast events only | ✅ Required | ❌ Optional | ✅ Required (schemas) |
| RPC (request/response) | ✅ Required | ✅ **Required** | ✅ Required (schemas) |
| With Management UI | ✅ Required | ✅ Recommended | ✅ Required (schemas + UI) |
...
**Use Case:** Teams that already have RabbitMQ/Redis/PostgreSQL
...
CELERYSALT_REDIS_URL = "redis://localhost:6379"
CELERYSALT_POSTGRES_URL = "postgresql://localhost/celerysalt"
```

**No Docker, no UI** - just the library connecting to existing infrastructure.
...
services:
  ...
  ui:
    image: celerysalt/ui:latest  # Free version
    # OR
...
**Important Notes:**
- **Redis is required if you use RPC events** (Celery needs it for result storage)
- For broadcast-only deployments, Redis is optional but recommended for caching
- RabbitMQ handles message routing, Redis handles result storage
